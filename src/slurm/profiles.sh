...
adaptation_profile_defaults() {
  ...
  case "$profile" in
    test)
      ...
      ;;
    screen|horizon_baselines_ablation|convex_baselines_ablation|delta_baselines_ablation|catboost_ablation|training_loss_ablation)
...
requires_selected_methods() {
  case "${EXPERIMENT_FAMILY:-}" in
    benchmark|mixed_quantity_ablation|fourier_retrieval_ablation|offline_datastore_ablation|retrieval_scope_ablation|horizon_baselines_ablation|convex_baselines_ablation|delta_baselines_ablation|catboost_ablation|training_loss_ablation|k_ablation|h_ablation|l_ablation|crossrag|sota_benchmark) return 0 ;;
    *) return 1 ;;
  esac
}
