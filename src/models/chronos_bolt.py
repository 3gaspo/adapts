...
"""Chronos-Bolt architecture and extraction wrapper.
...
autoregressive extension beyond the checkpoint's native horizon are omitted
because this project performs frozen inference at the checkpoint horizon.
"""
...
def _default_weights_path() -> Path | None:
    path = Path(__file__).resolve().parents[2] / "weights" / "chronos-bolt-base"
    return path.resolve() if path.exists() else None


class ChronosBolt(nn.Module):
    """Frozen project wrapper around the released Chronos-Bolt architecture."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.horizon > int(config.chronos_config["prediction_length"]):
            raise ValueError(
                "the local Chronos-Bolt implementation supports inference only "
                "up to the checkpoint prediction length"
            )
        dtype_key = (
            "dtype"
        ...
        self.model = ChronosBoltModelForForecasting.from_pretrained(
            str(model_path),
            config=config,
            local_files_only=bool(local_files_only),
            **{dtype_key: "auto"},
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        outputs = self.model(context=flat)
        median_index = int(
            torch.abs(self.model.quantiles - self.quantile_level).argmin().item()
        )
        prediction = outputs.quantile_preds[
            ...,
            median_index,
            : self.horizon,
        ]
