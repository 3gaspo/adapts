...
def _vanilla_metric_rows(payloads: dict[str, dict[str, Any]], eps: float = 1e-8) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for split, payload in payloads.items():
        pred = payload[f"{split}_preds"].float()
        target = payload[f"{split}_Y_values"].float()
        lookback = payload[f"{split}_X_values"].float()
        if pred.numel() == 0:
            continue
        err = pred - target
        scale = lookback.std(dim=-1, keepdim=True, unbiased=False).clamp_min(float(eps))
        rows.append(
            {
                "split": split,
                "baseline": "vanilla",
                ...
                "nmse": float((err / scale).pow(2).mean().item()),
            }
        )
    return rows
...
def main() -> dict[str, Path]:
    ...
    prediction_payloads["eval"] = eval_payload
    LOGGER.info("extraction done split=eval")
    if args.neighbors == 0:
        metrics_path = out / "vanilla_metrics.json"
        metrics_path.write_text(
            json.dumps(_vanilla_metric_rows(prediction_payloads), indent=2),
            encoding="utf-8",
        )
        LOGGER.info("vanilla metrics saved path=%s", metrics_path)
