"""Fit and evaluate gates between vanilla and context-conditioned forecasts."""
...
def flatten_payload(payload: dict[str, Any], prefix: str) -> dict[str, np.ndarray]:
    ...
    y_c = neighbor_to_query_scale(x, x_c, y_c_raw) if x_c.shape[2] > 0 else y_c_raw
    e = (
...
def distance_weights(arrays: dict[str, np.ndarray], eps: float = 1e-8) -> np.ndarray:
    ...
    if d.shape[1] == 0:
        raise ValueError("gate evaluation requires at least one retrieved neighbor")
...
COMMON_GATE_FEATURE_NAMES = (
    ...
    "weighted_neighbor_minus_vanilla_std",
...
def common_gate_features(arrays: dict[str, np.ndarray]) -> list[np.ndarray]:
    ...
    weighted_delta = weighted - pred
    distance = arrays["distance"]
    return [
        weighted_delta.mean(axis=1),
        weighted_delta.std(axis=1),
...
def scalar_gate_features(arrays: dict[str, np.ndarray]) -> np.ndarray:
    context_delta = arrays["pred_c"] - arrays["pred"]
    cols = [
    ...
    *,
    ...
    try:
        ...
        raise ModuleNotFoundError(
            "CatBoost gates require the `catboost` dependency."
        ) from exc
...
def fit_gate(
    ...
    *,
    ...
    if x_np.shape[0] == 0:
        raise ValueError("cannot train gates from an empty gate_train split")
    targets = np.asarray(y_np)
    if targets.ndim == 1:
        targets = targets[:, None]
    if objective not in {"classifier", "regressor"}:
        raise ValueError(f"unknown gate objective {objective!r}")
    fit_one = fit_improvement_classifier if objective == "classifier" else fit_loss_difference_regressor
    return [
        fit_one(
            x_np,
            targets[:, output_idx],
...
def fit_no_feature_context_gates(gate_train_arrays: dict[str, np.ndarray]) -> dict[str, np.ndarray | float]:
    """Bayes-optimal deterministic context decisions with no input features."""
    base_loss = (gate_train_arrays["y"] - gate_train_arrays["pred"]) ** 2
    context_loss = (gate_train_arrays["y"] - gate_train_arrays["pred_c"]) ** 2
...
def add_context_gate_predictions(
    ...
    gate_train_arrays: dict[str, np.ndarray],
    ...
    *,
    ...
    train_features = {
        "scalar": scalar_gate_features(gate_train_arrays),
        "horizon": horizon_gate_features(gate_train_arrays),
    }
    no_feature_artifacts = fit_no_feature_context_gates(gate_train_arrays)
    ...
    for split, arrays in arrays_by_split.items():
        ...
        for objective in ("classifier", "regressor"):
            for shape in ("scalar", "horizon"):
                ...
                split_predictions[name] = np.where(decision, arrays["pred_c"], arrays["pred"])
    ...
    artifacts = {
        ...
        "horizon_feature_names": horizon_gate_feature_names(gate_train_arrays["y"].shape[1]),
        "models": models,
...
def visualization_payload(
    ...
    return {
        "format_version": 2,
        "description": "Precomputed context-gate predictions and diagnostics for visualization.",
        "splits": splits,
    }
...
def write_metric_outputs(frame: pd.DataFrame, output_dir: Path) -> tuple[Path, Path]:
    csv_path = output_dir / "gate_metrics.csv"
    json_path = output_dir / "gate_metrics.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--gate-train-prefix", default="gate_train")
    parser.add_argument("--eval-prefix", default="eval")
...
def main() -> dict[str, Path]:
    ...
    output_dir = Path(args.output_dir).expanduser() if args.output_dir else input_dir / "gates"
    output_dir.mkdir(parents=True, exist_ok=True)
    LOGGER.info("experiment start kind=gates input=%s", input_dir)

    prefixes = [args.gate_train_prefix, args.eval_prefix]
    ...
    predictions_by_split, gate_artifacts, gate_diagnostics = add_context_gate_predictions(
        ...
        arrays_by_split[args.gate_train_prefix],
        arrays_by_split,
    ...
    LOGGER.info("evaluation start split=%s", args.eval_prefix)
    rows = evaluate_predictions(
        args.eval_prefix,
        arrays_by_split[args.eval_prefix],
        predictions_by_split[args.eval_prefix],
    )
    LOGGER.info("evaluation done rows=%s", len(rows))
    frame = pd.DataFrame(rows)
    artifact_path = output_dir / "gate_artifacts.pt"
    visualization_path = output_dir / "visualization_payload.pt"
    csv_path, json_path = write_metric_outputs(frame, output_dir)
    saved_artifacts: dict[str, Any] = {
        ...
        "gate_config": {
            ...
            "horizon_feature_names": horizon_gate_feature_names(
                arrays_by_split[args.gate_train_prefix]["y"].shape[1]
            ),
            "iterations": args.gate_iterations,
            "learning_rate": args.gate_learning_rate,
            "depth": args.gate_depth,
            "shapes": ["scalar", "horizon"],
            "gate_train_prefix": args.gate_train_prefix,
    ...
    torch.save(visualization_payload(predictions_by_split, gate_diagnostics), visualization_path)
