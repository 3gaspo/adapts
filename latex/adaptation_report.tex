...
\title{\vspace{-1.5em}\textbf{Retrieval-Based Adaptation of \\Time Series Foundation Models}\vspace{-0.4em}}
...
\begin{abstract}
This report describes my current work on retrieval-based adaptation for pretrained time series forecasting models. We briefly introduce notations,
the literature context, and the implemented baselines as well as the proposed TS-IFA architecture.
\end{abstract}
...
\subsection{Dynamically aligned datastores}

Let $P$ be the retrieval period and $D_{\rm stride}$ the datastore stride,
with $P\mid D_{\rm stride}$. Before optional explicit history bounds, define
the admissible endpoint pair
\[
 (a_s,b_s)=
 \begin{cases}
 ...
 \end{cases}
\]
The first phase-aligned candidate is
$r_s^{\rm align}=a_s+((s-a_s)\bmod P)$, and the uncapped date grid is
\[
 \mathcal G_s=\{r_s^{\rm align}+\ell D_{\rm stride}:\ell\in\mathbb N_0,\
 r_s^{\rm align}+\ell D_{\rm stride}\leq b_s\}.
\]
...
\]
Thus every $r\in\mathcal R_s$ satisfies
\[
 (s-r)\bmod P=0.
\]
...
Without an explicit override, the code first counts the phase-aligned dates
that a fixed $\Tzero$ store would provide for the current query. The online
...
otherwise. An implicit datastore stride is rounded upward to a multiple of
$P$.
...
content hash, and applied keys are recorded in the extraction manifest.
...
\]
The default candidate bank is
$\mathcal C=(c_0,c_1,c_2,c_3)=(p,p^c,c_r,c_m)$.
The rooters exclude $c_0$ and operate on the three non-vanilla deltas
$d_j=c_j-p$, $j\in\{1,2,3\}$. Fixed and learned transformed covariates are a
...
\]
For the learned type embedding $\tau_j\in\reals^{d_g}$ of delta $d_j$,
with $j\in\{1,2,3\}$,
let $\operatorname{LN}$ denote dimension-preserving layer normalization and
let $S_\omega:\reals^{2d_g+2H}\to\reals^H$ be the shared MLP scorer. It returns
the unconstrained horizon vector
\begin{align}
 a_j&=S_\omega\!\left([
 \operatorname{LN}(z_g),\operatorname{LN}(p),
 \operatorname{LN}(d_j),\operatorname{LN}(\tau_j)
 ]\right)\in\reals^H,\\
 \widehat Y_{\rm neural}&=p+\sum_{j=1}^{3}a_j\odot d_j,
 \qquad a_j\in\reals^H.
\end{align}
Here $\omega$ denotes all neural-rooter parameters: $A_g$, its layer
normalizations, the type embeddings, and the shared scorer.
There is no sigmoid, softmax, or convexity constraint. The vanilla fallback is
$a_j=0$ for every candidate.

The ridge rooter is horizon-wise on the same candidate deltas. In the joint
variant its free coefficient matrix is $\nu\in\reals^{3\times H}$ and is updated
...
\]
where $\|\cdot\|_F$ is the Frobenius norm. The ridge prediction is
$p_{i,h}+\sum_{j=1}^{3}\nu_{j,h}d_{i,j,h}$. Both ridge variants have exactly
$3H$ coefficients; $\alpha=10^{-2}$ applies to meta-ridge solves only.
...
\end{align}
differentiating through each exact standardized $3\times3$ solve. It freezes
...
256, and 10,000 one-random-batch epochs outside smoke mode. The exact variant,
...
$\nu^*_{\mathcal S_1^{(e)}}(\theta_b)$ exactly by the preceding $3\times3$
...
learning rate $10^{-5}$, weight decay $10^{-4}$, batch size 256, and 10,000
...
The active configuration uses four attention heads, per-head dimension 32,
128-unit hidden layers, and three rooter deltas. $K$ affects compute and memory,
not parameter count.

\begin{table}[H]
\centering
\small
\caption{Exact current TS-IFA parameter counts, reproduced by the implementation
and synchronized \code{config.json} artifacts.}
...
\code{crossrag} & four primary & $512{:}64$ & Chronos-Bolt
& min--max cosine, $K=15$; one winner vs. released Cross-RAG \\
\bottomrule
\end{tabularx}
\end{table}
...
Default publication strides are 24 pooled-adaptation query dates and 128
evaluation query dates; the datastore stride is 24 before period rounding and
the cap is 30,000 windows. The test profile uses adaptation/evaluation stride
256, datastore stride 168, and 2,048 store windows. Seed 1 is used throughout.
...
\code{crossrag.slurm} & one selected pipeline versus Cross-RAG at $(L,H,K)=(512,64,15)$ \\
\code{ts_ifa\_joint\_ridge.slurm} & joint-gradient ridge TS-IFA; current focus \\
\code{ts_ifa\_joint\_neural.slurm} & joint-gradient neural TS-IFA \\
\code{ts_ifa\_meta\_ridge.slurm} & exact support-solve ridge meta-TS-IFA \\
\code{ts_ifa\_meta\_neural.slurm} & gradient-inner-loop neural meta-TS-IFA \\
...
The completed screen selected six shared-ridge baselines, two hard shared
CatBoost regressors, and one shared Bayes gate; no convex or delta baseline was
eligible for that screen. Its extraction payloads and controls remain valid.
The convex, delta-ridge, and CatBoost formulation fronts are also complete and
did not replace those controls. The next study expands the retained candidates
over $K$; the screen fits need not be recomputed. Publication CatBoost fits
...
\code{TS-IFA} directory is not a valid input; each current TS-IFA variant owns
