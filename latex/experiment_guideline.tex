...
\begin{abstract}
This guideline is the current specification for retrieval-based adaptation of
pretrained time-series forecasters. It defines the notation, leakage controls,
retrieval contract, implemented baselines, TS-IFA variants, experiment grids,
artifacts, and practical workflow. Obtained results are maintained separately
in \code{executive_summary.tex}.
\end{abstract}
...
Every run uses \code{schema_version: 1}, input fingerprints, launch attempts,
required artifacts, and status not-run/running/interrupted/completed. The
default \code{overwrite_exact} collision policy skips exact completion,
resumes exact interruption, and adds \code{run_n} for a changed pipeline.
Tables support distinct/latest/selected/average pipeline configurations and
selected/latest/distinct/average exact repeats. Explicit pipeline filters must
match even with one run. \code{SELECTED_RUNS.txt} records automatic or pinned
selection, and every report records its exact manifests. Unmigratable legacy
material is isolated under
\code{outputs/archive/legacy_pre_schema_v1_2026-08-07} and is never read by
current tables.
