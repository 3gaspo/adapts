...
artifact selection or staging. It then selects only the exact stdout/stderr pair and run/report roots whose
manifests record that launch ID, excludes \code{*.pt}, \code{*.npy}, and
\code{*.cbm}, commits only those paths on \code{main}, and pushes
\code{origin/main} without creating a pull request. Omitting the job ID instead
publishes the complete log and lightweight output trees. A non-fast-forward
