...
The sole manual follow-up selection is the root
\code{SWEEP_CANDIDATES.txt}, whose non-comment records are complete
\code{family/retrieval/method} identities. A shared reader filters adaptation,
shared-ridge, shared-CatBoost, TS-IFA ridge, or TS-IFA neural entries for each
front. \code{SELECTED_CANDIDATES_FILE} may select another manifest; the
front-specific CSV variables remain explicit one-launch overrides. TS-IFA H/L
and meta fronts require at least one matching \code{ts_ifa} entry or override.

The corrected current screen ranking compares each adapted result only with the
vanilla result from the same retrieval run. The manual follow-up manifest holds
the top five shared-ridge pipelines, the best shared CatBoost cov/avgY pipeline,
and the matching shared Bayes references. No convex or delta baseline was
eligible for the screen. Earlier formulation and $K$ summaries derived from the
old cross-retrieval ranking remain invalid; the current formulation reports
below were regenerated through the corrected reader. The incomplete $K$ sweep
still supports no optimum. Publication CatBoost fits
...
Every baseline has its largest equal-dataset mean nMSE gain at $L=24$, but that
aggregate is not a stable selection criterion: the Traffic vanilla reference at
$L=24$ has nMSE $2.07\times10^{10}$ and drives the scale pathology already
identified in the synchronized report. At $L=168$, all five baselines remain
positive and the three instance-ridge designs cluster at +0.90--1.14\%; at
$L=504$, the full designs fall to +0.24--0.41\% and all window-win averages are
below 50\%. We therefore retain $L=168$ as the balanced $H=24$ setting rather
than promoting the numerically larger but outlier-sensitive $L=24$ aggregate.
...
script selects only the exact stdout/stderr pair and run/report roots whose
manifests record that launch ID, excludes \code{*.pt}, \code{*.npy}, and
\code{*.cbm}, and commits those paths on \code{main}. It then sources
\code{proxy.sh} with \code{.secrets/proxy.credentials} from
\code{$HOME/codes/} and pushes \code{origin/main}
without pulling. Omitting the job ID instead publishes the complete log and
lightweight output trees. GitHub authentication remains separate from proxy
authentication and is handled by the configured credential helper or the
...
baseline with instance-normalized Euclidean retrieval and $K=3$. Fourier
