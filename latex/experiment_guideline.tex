...
remain available to the adapter \citep{chronos,chronoscode}. Source-faithful
local model modules load the released base weights from
\path{weights/chronos-bolt-base} and the sole recursive \code{best.pth} below
\path{weights/cross-rag}; \path{CHRONOS_BOLT_WEIGHTS_PATH} and
\path{CROSSRAG_WEIGHTS_PATH} are explicit overrides. No external code checkout
or Cross-RAG retraining is part of the workflow.
...
rules in \cref{sec:practical}. For scope
$q\in\{\mathrm{all},\mathrm{same},\mathrm{other}\}$, the eligible store keys are
\[
 \mathcal I^{\rm all}_{s,u}=\mathcal R_s\times\mathcal U,\qquad
 \mathcal I^{\rm same}_{s,u}=\mathcal R_s\times\{u\},\qquad
 \mathcal I^{\rm other}_{s,u}=\mathcal R_s\times(\mathcal U\setminus\{u\}).
\]
The labelled datastore $\mathcal D^q_{s,u}$ contains
$(\phi(X_{r,v}),X_{r,v},Y_{r,v},r,v)$ for every
$(r,v)\in\mathcal I^q_{s,u}$.
For $K\in\{1,\ldots,|\mathcal I^q_{s,u}|\}$, let $\TopK^{\min}$ return the ordered
$K$ minimizers. Exact search returns the following store keys:
\[
 \mathcal N^q_K(s,u)
 =\TopK_{(r,v)\in\mathcal I^q_{s,u}}^{\min}
...
\label{tab:profiles}
\begin{tabularx}{\linewidth}{@{}p{.17\linewidth}p{.17\linewidth}p{.16\linewidth}p{.13\linewidth}X@{}}
\toprule
Profile & Datasets & $(L,H)$ & Backbone & Retrieval / methods \\
\midrule
\code{test} & Electricity & $504{:}168$ & Chronos-2 & raw Euclidean, $K=3$; smoke-only \\
\code{screen} & four primary & \shortstack{$168{:}24$, $336{:}48$,\\$504{:}168$} & Chronos-2
...
\code{sota_benchmark} & seven paper datasets & $512{:}64$ & Chronos-Bolt
& selected method on exact paper test splits plus immutable published TS-RAG/Cross-RAG rows \\
\code{tsrag} & four primary & $512{:}64$ & Chronos-2, Chronos-Bolt
& fixed Chronos-T5 Euclidean retrieval at $K=10$; released TS-RAG is Bolt-only \\
\bottomrule
\end{tabularx}
\end{table}
...
\code{full} & raw, 1 & ridge shared / horizon & ridge / convex shared & ridge / delta shared \\
\code{cov_y} & instance, 3 & ridge shared / horizon & ridge / convex shared & ridge / delta shared \\
\code{full} & raw, 3 & ridge shared / horizon & ridge / convex shared & ridge / delta shared \\
\code{cov_avgy} & instance, 3 & ridge shared / horizon & ridge / convex shared & ridge / delta shared \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
\code{avgy} & instance, 1 & regressor shared & classifier shared; regressor shared soft; regressor horizon \\
\code{cov} & raw, 1 & regressor shared & classifier shared; regressor shared soft; regressor horizon \\
\bottomrule
\end{tabularx}
\end{table}
...
\code{k_ablation.slurm} & selected pipelines, $K\in\{1,3,5,10,15,20\}$ only \\
...
\code{horizon_baselines_ablation.slurm} & five controls in \cref{tab:selected-baselines}; shared versus horizon ridge \\
\code{convex_baselines_ablation.slurm} & same five; shared ridge versus shared convex \\
\code{delta_baselines_ablation.slurm} & same five; shared ridge versus shared delta-ridge \\
...
\code{fourier_retrieval_ablation.slurm} & selected pipelines under their original and Fourier-amplitude retrieval spaces \\
\code{offline_datastore_ablation.slurm} & selected pipelines under online and fixed T0-only retrieval \\
\code{retrieval_scope_ablation.slurm} & selected pipelines with all-user, same-user-only, and other-users-only retrieval \\
\code{sota_benchmark.slurm} & selected Chronos-Bolt method on exact paper splits plus immutable published TS-RAG/Cross-RAG rows \\
\code{tsrag.slurm} & released TS-RAG and the selected project method on matching fixed $K=10$ neighbours \\
...
Manual follow-up selection has two immutable generations. The root
\code{SWEEP_CANDIDATES.txt} records the screen-selected pipelines used by the
already-launched formulation, retrieval, K/H/L, and mixed-panel studies. After
the K study is complete and analyzed, promoted best-K adaptation identities are
written to \path{SECOND_GENERATION_CANDIDATES.txt}; this second manifest alone
feeds the final benchmark, TS-RAG, and SOTA fronts. Selected TS-IFA identities
are appended to it only after the routing pilot is analyzed, before the TS-IFA
H/L and meta follow-ups. Non-comment records in both files are complete
\code{family/retrieval/method} identities, and the shared reader filters the
applicable adaptation, shared-ridge, shared-CatBoost, TS-IFA ridge, or TS-IFA
neural entries for each front. \code{SELECTED_CANDIDATES_FILE} may select
another manifest; front-specific CSV variables remain explicit one-launch
overrides. A consumer fails clearly while its required generation-2 entries are
absent.

The corrected current screen ranking compares each adapted result only with the
vanilla result from the same retrieval run. The frozen first-generation
manifest holds the top five shared-ridge pipelines, the best shared CatBoost
cov/avgY pipeline, and the matching shared Bayes references. The completed
$K$ study promotes eight unique generation-2 identities in decreasing order of
their best-$K$ held-out improvement; the two first-generation raw-full rows
collapse to one identity. The first record is
\code{baselines/instance_euclidean_5_online/full_ridge_shared}, so the final
benchmark evaluates all eight records while TS-RAG and SOTA use that leading
shared-ridge control. Bayes ties retain the smallest $K=1$. No convex or delta
baseline was eligible for the screen. Earlier formulation and $K$ summaries
derived from the old cross-retrieval ranking remain invalid; the current
reports below were regenerated through the corrected reader. Publication
CatBoost fits allow 300 trees; only the smoke profile uses two.

\subsection{Current held-out results}

All results in this subsection are complete held-out $\Tthree$ comparisons
against the vanilla forecast from the same retrieval run. Improvements are the
equal-configuration mean $I_{m,c}$ defined below; window wins are the mean of
$W_{m,c}$. The screen contains 12 dataset--setting configurations.

\begin{table}[H]
\centering
\small
\caption{Leading pipelines from the corrected shared-method screen. The five
ridge rows are the selected baseline sweep; the final row is the strongest
deployable learned gate.}
\label{tab:current-screen-results}
\begin{tabularx}{\linewidth}{@{}Xlrrr@{}}
\toprule
Method & Retrieval & $K$ & Mean $I_{m,c}$ & Window wins \\
\midrule
\code{full_ridge_shared} & instance Euclidean & 3 & \textbf{+1.95\%} & 53.18\% \\
\code{full_ridge_shared} & raw Euclidean & 1 & +1.79\% & 53.40\% \\
\code{cov_y_ridge_shared} & instance Euclidean & 3 & +1.78\% & 53.65\% \\
\code{full_ridge_shared} & raw Euclidean & 3 & +1.78\% & 52.64\% \\
\code{cov_avgy_ridge_shared} & instance Euclidean & 3 & +1.76\% & \textbf{54.17\%} \\
\midrule
\code{catboost_cov_regressor_shared} & raw Euclidean & 1 & +0.03\% & 3.50\% \\
\bottomrule
\end{tabularx}
\end{table}

The baseline sweep therefore selects shared ridge, not a gate. The full design
with instance-normalized retrieval and $K=3$ gives the largest mean reduction,
while all five selected ridge pipelines lie in the narrow +1.76--1.95\% band.
The CatBoost gate is effectively tied with vanilla and improves too few windows
to be a competitive primary adaptor.

\begin{table}[H]
\centering
\scriptsize
\caption{Matched one-axis retrieval and formulation ablations. Each entry is
mean held-out nMSE improvement over its retrieval-matched vanilla run. The
shared column is the unchanged control in the dedicated ablation reports.}
\label{tab:current-formulation-results}
\begin{tabularx}{\linewidth}{@{}p{.25\linewidth}rrrrr@{}}
\toprule
Control pipeline & Shared & Fourier & Horizon & Convex & Delta \\
\midrule
\code{full}, instance, $K=3$ & \textbf{+1.95} & +1.33 & +1.52 & $-0.05$ & +0.61 \\
\code{full}, raw, $K=1$ & \textbf{+1.79} & +1.28 & +1.33 & +0.55 & +0.57 \\
\code{cov_y}, instance, $K=3$ & \textbf{+1.78} & +1.37 & +1.35 & +0.71 & +0.47 \\
\code{full}, raw, $K=3$ & \textbf{+1.87} & +1.33 & +1.45 & +0.88 & +0.82 \\
\code{cov_avgy}, instance, $K=3$ & \textbf{+1.76} & +1.62 & +1.43 & +0.76 & +0.54 \\
\bottomrule
\end{tabularx}
\end{table}

The original Euclidean retrieval beats the Fourier-amplitude alternative for
every selected pipeline. Shared ridge also beats its matched per-horizon,
convex, and delta-ridge replacement in every row. The extra horizon parameters
do not compensate for their higher variance, and the convex and delta
constraints remove useful signed fusion freedom. These alternatives remain
ablation-only; shared ridge is the sole retained baseline formulation.

\begin{table}[H]
\centering
\small
\caption{Completed baseline lookback ablation at fixed $H=24$. Each cell reports
mean held-out nMSE improvement / mean positive-window rate (\%), averaged
equally over Electricity, Traffic, Solar, and Exchange.}
\label{tab:current-lookback-results}
\begin{tabularx}{\linewidth}{@{}Xrrr@{}}
\toprule
Pipeline & $L=24$ & $L=168$ & $L=504$ \\
\midrule
\code{full}, raw, $K=3$ & +6.72 / 55.34 & +1.04 / 49.70 & +0.24 / 48.78 \\
\code{full}, raw, $K=1$ & +4.49 / 51.89 & +0.81 / 50.05 & +0.37 / 49.14 \\
\code{full}, instance, $K=3$ & +2.43 / 53.03 & \textbf{+1.14} / 49.85 & +0.41 / 48.64 \\
\code{cov_y}, instance, $K=3$ & +2.90 / 53.05 & +0.90 / 50.40 & +0.97 / 49.02 \\
\code{cov_avgy}, instance, $K=3$ & +2.92 / 52.35 & +0.93 / 50.65 & \textbf{+1.03} / 49.50 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=.96\linewidth]{l_ablation_baseline_results.pdf}
\caption{Baseline lookback-length ablation. Dashed references mark no nMSE
improvement and a 50\% window win rate.}
\label{fig:current-lookback-results}
\end{figure}

\begin{table}[H]
\centering
\small
\caption{Lookback and horizon sensitivity of the screened
\code{full_ridge_shared} instance-$K=3$ control. Entries are equal-dataset mean
held-out nMSE improvements (\%) over the retrieval-matched vanilla forecast.}
\label{tab:current-lh-sensitivity}
\begin{tabular}{@{}llrrr@{}}
\toprule
Varied axis & Fixed axis & 24 & 168 & 504 \\
\midrule
Lookback $L$ & $H=24$ & +2.43 & +1.14 & +0.41 \\
Horizon $H$ & $L=504$ & +0.41 & \textbf{+2.78} & +2.75 \\
\bottomrule
\end{tabular}
\end{table}

The apparent $L=24$ maximum is not stable: its dataset effects range from
$-7.37\%$ on Exchange to $+16.48\%$ on Solar, while the Traffic vanilla nMSE is
$2.07\times10^{10}$. We therefore retain $L=168$ as the balanced short-horizon
setting. At fixed $L=504$, the gain rises from +0.41\% at $H=24$ to +2.78\% at
$H=168$ and then plateaus at +2.75\% for $H=504$, showing that the adaptor's
relative benefit persists at long horizons.

\begin{table}[H]
\centering
\small
\caption{Transfer of the best screened \code{full_ridge_shared} instance-$K=3$
pipeline to panels whose channels mix physical quantities. The primary row is
the homogeneous screen reference; mixed rows average three $(L,H)$ settings.}
\label{tab:current-mixed-results}
\begin{tabularx}{\linewidth}{@{}Xrr@{}}
\toprule
Panel & Mean $I_{m,c}$ & Window wins \\
\midrule
Primary homogeneous screen & \textbf{+1.95\%} & 53.18\% \\
ETTh1 mixed & +3.08\% & 52.88\% \\
ETTh2 mixed & +1.19\% & 49.98\% \\
ETTm1 mixed & +8.60\% & 52.58\% \\
ETTm2 mixed & +0.91\% & 53.59\% \\
Weather mixed & $-125.87$\% & 42.78\% \\
\midrule
All five mixed panels & $-22.42$\% & 50.36\% \\
\bottomrule
\end{tabularx}
\end{table}

Some ETT panels remain locally adaptable, but mixing unrelated variables makes
the method non-robust: Weather reverses the aggregate result, and raw-retrieval
full designs additionally exhibit catastrophic ETTm1 outliers. Cross-variable
retrieval and fusion should therefore be restricted to channels representing
the same physical quantity. Mixed-quantity panels are diagnostic ablations, not
members of the primary benchmark.
...
Downstream allocation resolves the extraction dependency by its complete
expected pipeline configuration and exact seed and fails unless precisely one
run remains. Its signature embeds the upstream schema, identity, pipeline and
experiment configuration, and seeds. The upstream path and manifest ID remain
plain provenance and never affect reuse. Thus a smoke extraction cannot satisfy
a publication computation merely because both occupy the same identity path.
...
seed state is \code{ready}; completion is recorded immediately after that
producer's \code{srun} returns successfully. A later configuration or table
failure preserves completed producers and interrupts only unfinished work. Completed manifests are
authoritative and are not followed by file hashing or synchronization checks.
Within that active workflow, later stages may select ready manifests only when
their launch ID matches; independent reuse remains completed-only.
...
\code{bash publish_job.sh <job-id>} from the project root. The
script verifies \code{main}, sources \code{$HOME/codes/proxy.sh} (or
\code{PROXY_SCRIPT_PATH}), and fast-forward pulls \code{origin/main} before
artifact selection or staging. With a job ID, it commits only the exact
stdout/stderr pair. Omitting the job ID publishes the complete log and
lightweight output trees, excluding \code{*.pt}, \code{*.npy}, and
\code{*.cbm}. It then pushes \code{origin/main} without creating a pull request.
A non-fast-forward
pull stops without creating a merge commit; unrelated staged paths remain
outside the publisher commit.
...
with backbone quality. Current complete evidence selects the shared full ridge
baseline with instance-normalized Euclidean retrieval and $K=5$; its completed
$K$ sweep gives a +2.00\% equal-configuration mean nMSE improvement. Fourier
retrieval and per-horizon, convex, or delta formulations are uniformly weaker
in matched comparisons, while mixed-quantity panels are unstable enough to
justify homogeneous-quantity grouping as the primary contract.

\bibliographystyle{plainnat}
\bibliography{adaptation_references}
